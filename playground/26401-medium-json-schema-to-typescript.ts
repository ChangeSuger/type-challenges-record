...
type JSONSchema = {
  type: keyof PrimitiveType | 'object' | 'array'
  enum?: any[]
  properties?: {
...
type JSONSchema2TS<T extends JSONSchema> =
  T['type'] extends keyof PrimitiveType
  ? T['enum'] extends any[]
    ? T['enum'][number]
    : PrimitiveType[T['type']]
  : T['type'] extends 'object'
    ? T['properties'] extends { [key: string]: JSONSchema }
      ? T['required'] extends string[]
        ? Omit<
            { [k in keyof T['properties'] as k extends T['required'][number] ? never : k]?: JSONSchema2TS<T['properties'][k]>} &
            { [k in keyof T['properties'] as k extends T['required'][number] ? k : never]: JSONSchema2TS<T['properties'][k]>},
            never
          >
