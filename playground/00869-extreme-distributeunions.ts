...
type DistributeUnions<T> =
  T extends any[]
  ? DistributeUnionsForTuple<T>
  : T extends object
    ? Merge<DistributeUnionsForObject<T>>
    : T

type DistributeUnionsForTuple<T extends unknown[]> =
  T extends [infer L, ...infer R]
  ? UnionsTupleHander<DistributeUnions<L>, R>
  : []

type UnionsTupleHander<L, R extends unknown[]> = L extends L ? [L, ...DistributeUnionsForTuple<R>] : never

type DistributeUnionsForObject<T extends object, K extends keyof T = keyof T> =
  [K] extends [never]
  ? unknown
  : K extends K
    ? UnionsObjectHander<K, DistributeUnions<T[K]>> & DistributeUnionsForObject<Omit<T, K>>
    : unknown

type UnionsObjectHander<K, U> = U extends U ? {
  [k in K & string]: U
} : never
