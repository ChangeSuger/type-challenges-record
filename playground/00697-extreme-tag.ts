...
type TagsBag<B, T extends string[]> = {
  [UniqueSymbol]?: UniqueSymbolType | (UniqueSymbolType & [B, T])
}

type GetTags<B> =
  Equal<B, never> extends true
  ? []
  : B extends TagsBag<unknown, infer T extends string[]>
    ? Equal<T, string[]> extends true
      ? []
      : T
    : []

type PassUnvalidValue<B, D> =
...
type Tag<B, T extends string> = PassUnvalidValue<B, UnTag<B> & TagsBag<UnTag<B>, [...GetTags<B>, T]>>

type UnTag<B> = PassUnvalidValue<B, Omit<B, UniqueSymbolType>>

type HasTag<B, T extends string> = T extends GetTags<B>[number] ? true : false
type HasTags<B, T extends readonly string[]> = T[number] extends GetTags<B>[number] ? true : false
type HasExactTags<B, T extends readonly string[]> = Equal<T[number], GetTags<B>[number]>
